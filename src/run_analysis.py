...
from generate_narrative import build_metrics_summary, call_llm, fallback_narrative
...
def main():
    ...
    # Insights report — static, rule-based summary (always available)
    report = build_insights(daily, weekly, by_item, pattern)
    with open(os.path.join(OUTPUT_DIR, "insights_report.txt"), "w") as f:
        f.write(report)

    # AI-generated executive narrative — self-writing "so what", regenerated
    # every run from the same computed metrics. Falls back automatically to
    # a static template if ANTHROPIC_API_KEY isn't set, so this never breaks
    # the pipeline.
    metrics = build_metrics_summary()
    try:
        narrative = call_llm(metrics)
        source = "Claude (claude-sonnet-4-6)"
    except Exception as e:
        print(f"LLM call skipped or failed ({e}); using fallback narrative.")
        narrative = fallback_narrative(metrics)
        source = "static fallback template"

    narrative_path = os.path.join(OUTPUT_DIR, "ai_generated_insights.md")
    with open(narrative_path, "w") as f:
        f.write("# AI-Generated Executive Summary\n\n")
        f.write(f"*Generated by: {source}*\n\n")
        f.write(narrative + "\n")
    ...
    print("AI narrative saved to:", narrative_path)
